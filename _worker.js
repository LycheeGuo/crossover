...
const CONST_ACADEMIC_PROXIES = [
    ...
    'http://172.86.75.243:9036',
    'http://78.26.211.61:9080',
    'http://201.217.28.203:88',
    ...
    async fetch(request, env, ctx) {
        ...
        // 订阅处理
            ...
            if (url.searchParams.get('token') === 订阅TOKEN) {
                ...
                // [重点修改] 使用新的生成逻辑
                const 完整优选列表 = (await 生成固定规则IP(25, -1))[0];
                ...
                if (url.searchParams.has('target') || ua.includes('clash') || ua.includes('meta')) {
                     const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${url.searchParams.get('target')||'clash'}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?token=' + 订阅TOKEN + '&sub')}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false`;
                     try {
                        const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter' } });
                        if (response.ok) return new Response(await response.text(), { status: 200, headers: { "content-type": "application/x-yaml; charset=utf-8" } });
                     } catch(e) {}
                }
        ...
        async write(chunk) {
            ...
            if (hostname.includes('scholar.google.com')) {
                ...
                try {
                    ...
                    console.error(`硬编码代理连接失败 (${e.message})，尝试切换到 AIP...`);
                }
                ...
                if (AIP_List.length > 0) {
                    try {
                        ...
                        console.error(`AIP 代理也连接失败: ${e.message}`);
                    }
                }
            }
            // -----------------------------------------------------------
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, defaultProxyIP) {
    ...
    try {
        const fallbackIP = atob('UFJPWFlJUC50cDEuMDkwMjI3Lnh5eg==');
...
// [生成固定规则IP函数] - 优化版
async function 生成固定规则IP(count = 25, 指定端口 = -1) {
...
async function surge(content, url, config_JSON) { return content; } // 简化版占位，非关键
