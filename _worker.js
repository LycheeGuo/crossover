...
async function 处理WS请求(request, yourUUID) {
    ...
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            const { port, hostname, rawClientData, rawIndex, version, isUDP, hasError } = parsedInfo;
            if (hasError) return; // 错误处理

            if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');

            // -----------------------------------------------------------
            // [新增] 核心分流逻辑 - 拦截 Scholar
            // -----------------------------------------------------------
            if (hostname.includes('scholar.google.com') && GOOGLE_SCHOLAR_PROXIES.length > 0) {
                // 准备数据：Trojan 使用 rawClientData, VLESS 使用 chunk 切片
                const dataToProxy = 判断是否是木马 ? rawClientData : chunk.slice(rawIndex);
                // 准备头部：Trojan 不需要 (null), VLESS 需要回写头部
                ...
                try {
                    ...
                    console.error('All Scholar proxies failed, falling back to regular connection');
                    // 失败则继续执行下方的常规逻辑
                } catch (e) {}
            }
            // -----------------------------------------------------------
...
async function connectToScholarProxy(targetHost, targetPort, initialData, ws, respHeader, remoteConnWrapper, proxyAddressString) {
    ...
    let newSocket;
    // 调用改造后的 connect 函数，传入 proxyConfig
...
////////////////////////////////SOCKS5/HTTP函数///////////////////////////////////////////////
// [修改] 增加 proxyConfig 参数，默认为 null (兼容旧逻辑)
async function socks5Connect(targetHost, targetPort, initialData, proxyConfig = null) {
    // 优先使用传入的配置，否则使用全局配置
...
// [修改] 增加 proxyConfig 参数，默认为 null (兼容旧逻辑)
async function httpConnect(targetHost, targetPort, initialData, proxyConfig = null) {
    // 优先使用传入的配置，否则使用全局配置
    const { username, password, hostname, port } = proxyConfig || parsedSocks5Address;
    const socket = connect({ hostname, port }), writer = socket.writable.getWriter(), reader = socket.readable.getReader();
    try {
