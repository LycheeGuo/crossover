...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null) {
	...
	async function 写入首包(remoteSock, data) {
		...
		try { await writer.write(数据转Uint8Array(data)) } finally { try { writer.releaseLock() } catch (e) { } }
	}
	...
	async function connectToScholar(允许发送首包 = true) {
		...
		const 当前连接任务 = (async () => {
			...
			// 备份原始全局解析配置
			const originalParsedAddress = parsedSocks5Address;
			...
			for (const proxy of proxiesToTry) {
				try {
					...
					// 临时覆盖，使原生 httpConnect 读取目标配置
					parsedSocks5Address = await 获取SOCKS5账号(proxyAddressStr, 80); 
			...
			// 恢复原始配置
			parsedSocks5Address = originalParsedAddress;
	...
	async function connecttoPry(允许发送首包 = true) {
		...
		const 当前连接任务 = (async () => {
			...
			if (启用SOCKS5反代 === 'socks5') {
				...
				newSocket = isIPHostname(parsedSocks5Address.hostname) ? await httpsConnect(host, portNum, 本次首包数据, TCP连接) : await httpConnect(host, portNum, 本次首包数据, true, TCP连接);
				...
				if (有效数据长度(本次首包数据) > 0) {
					...
					try { await writer.write(数据转Uint8Array(本次首包数据)) } finally { try { writer.releaseLock() } catch (e) { } }
				}
	...
	const 启动标准兜底策略 = async () => {
		if (启用SOCKS5反代 && (启用SOCKS5全局反代 || 验证SOCKS5白名单(host))) {
			log(`[TCP转发] 启用 SOCKS5/HTTP/TURN/SSTP 全局代理`);
			try {
				await connecttoPry();
			} catch (err) {
